(function(){// reusable function to reset application states
function reset() {
...
blog.route('/', {
	action: function() {
		if ( Meteor.isClient ) {
			...
			document.title = 'filip's notebook → blog';
			BlazeLayout.render('main', {content: 'home'});
		}
	}
});
...
blog.route('/:slug', {
	...
	action: function() {
		if ( Meteor.isClient ) {
			...
			if ( post ) {
				document.title = 'filip's notebook → ' + post.title;
			}
...
projects.route('/', {
	...
	action: function() {
		if ( Meteor.isClient ) {
			...
			document.title = 'filip's notebook → projects';
		}
	}
});
